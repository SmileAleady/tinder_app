...
class ChatLocalDb {
  ...
  Future<ConversationListData> getConversationListData({
    ...
    await ensureFreshNewMatches(activeUserId);
  ...
  Future<void> ensureFreshNewMatches(
    String activeUserId, {
    int targetCount = 4,
  }) async {
    final raw = await _readRaw();
    ...
    final relatedMatches =
        matches.where((m) => m.includes(activeUserId)).toList();
    final existingPeerIds = relatedMatches
        .map((m) => m.peerIdFor(activeUserId))
        .whereType<String>()
        .toSet();

    final currentNewCount = relatedMatches.where((match) {
      final pairMessages = messages.where((m) => m.pairId == match.pairId);
      return !match.openedBy.contains(activeUserId) && pairMessages.isEmpty;
    }).length;

    if (currentNewCount >= targetCount) {
      return;
    }

    final missing = targetCount - currentNewCount;
    final allUsers = await getAllUserProfiles();
    final available = allUsers
        .where(
          (u) => u.userId != activeUserId && !existingPeerIds.contains(u.userId),
        )
        .toList();

    final selectedPeerIds = <String>[];
    for (final user in available.take(missing)) {
      selectedPeerIds.add(user.userId);
    }

    if (selectedPeerIds.length < missing) {
      final generatedCount = missing - selectedPeerIds.length;
      for (var i = 0; i < generatedCount; i++) {
        final generated = await _createAutoGeneratedUser();
        selectedPeerIds.add(generated.userId);
      }
    }
    ...
    final now = DateTime.now();
    for (var i = 0; i < selectedPeerIds.length; i++) {
      final peerId = selectedPeerIds[i];
      final pairId = _pairId(activeUserId, peerId);
      if (matches.any((m) => m.pairId == pairId)) {
        continue;
      }
      matches.add(
        ChatMatchRecord(
          ...
          userA: activeUserId,
          userB: peerId,
          matchedAtIso: now.subtract(Duration(minutes: i)).toIso8601String(),
          openedBy: <String>[],
        ),
      );
    }
  ...
  Future<UserProfileModel> _createAutoGeneratedUser() async {
    final userId = await UserAuthLocalDb.instance.generateUniqueUserId();
    final rand = Random.secure();
    final suffix = userId.replaceFirst('user_', '');
    final nickname = _autoNicknames[rand.nextInt(_autoNicknames.length)];
    final city = _autoCities[rand.nextInt(_autoCities.length)];
    final age = 20 + rand.nextInt(12);
    final goal = _autoGoals[rand.nextInt(_autoGoals.length)];
    final gender = OptionDataManager.genders[rand.nextInt(OptionDataManager.genders.length)];
    final orientation = OptionDataManager
        .sexualOrientations[rand.nextInt(OptionDataManager.sexualOrientations.length)];

    final profile = UserProfileModel(
      userId: userId,
      email: 'auto_$suffix@match.local',
      phone: '',
      password: '',
      nikeName: nickname,
      mediaUrls: const [],
      smartPhotosEnabled: true,
      aboutMe: '喜欢认识新朋友，聊得来可以一起喝咖啡。',
      prompts: const [],
      interests: const [],
      relationshipGoal: goal,
      height: null,
      languages: const [UserLanguage(id: 'zh', name: '中文')],
    ...
    await UserAuthLocalDb.instance.upsertUser(profile);
    return profile;
  }
  ...
  static const List<String> _autoNicknames = [
    '小晚',
    '阿泽',
    '米粒',
    '木木',
  ...
  static const List<String> _autoCities = [
    '上海',
    '北京',
    '深圳',
    '杭州',
  ...
  static const List<UserRelationshipGoalItem> _autoGoals = [
    UserRelationshipGoalItem(id: 1, title: '认真交友', emoji: '💬'),
    UserRelationshipGoalItem(id: 2, title: '先聊聊看', emoji: '🙂'),
    UserRelationshipGoalItem(id: 3, title: '找搭子', emoji: '🌟'),
  ];
}
